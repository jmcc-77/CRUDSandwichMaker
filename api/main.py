...
def read_one_sandwich(sandwich_id: int, db: Session = Depends(get_db)):
    result = sandwiches.read_one(db, sandwich_id=sandwich_id)
    if result is None:
...
def read_one_resource(resource_id: int, db: Session = Depends(get_db)):
    result = resources.read_one(db, resource_id=resource_id)
    if result is None:
...
def update_one_order_detail(order_detail_id: int, order_detail: schemas.OrderDetailUpdate, db: Session = Depends(get_db)):
    order_detail_check = order_details.read_one(db, order_detail_id=order_detail_id)
    if order_detail_check is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
...
def delete_one_order(order_detail_id: int, db: Session = Depends(get_db)):
    order_detail_check = order_details.read_one(db, order_detail_id=order_detail_id)
    if order_detail_check is None:
        raise HTTPException(status_code=404, detail="Order detail not found")
